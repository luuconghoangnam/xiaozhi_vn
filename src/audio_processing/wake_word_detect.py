...
class WakeWordDetector:
    ...
    def _init_kws_model(self):
        ...
        try:
            ...
            keywords_path = self._prepare_keywords_file(tokens_path, keywords_path)
            if keywords_path is None:
                raise ValueError("没有可用的唤醒词，请检查 models/keywords.txt")
    ...
    def _prepare_keywords_file(self, tokens_path: Path, keywords_path: Path) -> Path | None:
        """
        Build a Sherpa-compatible keyword file.

        The bundled UI/docs often write lines like "n i h ao @你好".  This
        Sherpa model expects token-only lines, so labels/comments must be
        stripped before initializing the native KeywordSpotter.  Invalid tokens
        are skipped here to avoid native initialization failures.
        """
        try:
            ...
            with open(keywords_path, "r", encoding="utf-8") as f:
                for raw_line in f:
                    ...
                    # Labels/comments are for humans/UI only, not for Sherpa.
                    for marker in ("@", "#"):
                        if marker in line:
                            line = line.split(marker, 1)[0].strip()

                    # Some examples use per-keyword scores (" :1.5"); this
                    # runtime config already supplies score/threshold globally.
                    if " :" in line:
                    ...
                    if not parts:
                        continue
                    missing = [token for token in parts if token not in available_tokens]
                    if missing:
                        skipped_lines.append(
                            f"{raw_line.strip()} (missing tokens: {', '.join(missing)})"
                        )
                        continue
            ...
            runtime_keywords_path = self.model_dir / ".keywords.runtime.txt"
            runtime_keywords_path.write_text(
                "\n".join(valid_lines) + "\n", encoding="utf-8"
            )
            logger.info("已加载 %d 条有效唤醒词", len(valid_lines))
            return runtime_keywords_path
        except Exception as e:
            logger.error(f"准备唤醒词文件失败: {e}", exc_info=True)
            return None
    ...
    async def _process_audio(self):
        ...
        try:
            ...
            # Log peak volume every 5 seconds to debug microphone levels
            current_time = time.time()
            if not hasattr(self, "_last_vol_log_time"):
                self._last_vol_log_time = 0
            
            peak = np.max(np.abs(samples))
            if current_time - self._last_vol_log_time > 5.0:
                logger.info(f"KWS Mic Volume Peak: {peak:.4f}")
    ...
    def _validate_config(self):
        ...
        # 验证阈值参数 (允许更低的阈值以提高灵敏度)
        if not 0.01 <= self.keywords_threshold <= 1.0:
