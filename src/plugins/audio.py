...
class AudioPlugin(Plugin):
    ...
    def __init__(self) -> None:
        ...
        self._mute_until = 0  # Thời điểm hết hạn tắt mic tạm thời
    ...
    async def on_device_state_changed(self, state):
        ...
        if state == DeviceState.LISTENING:
            ...
            try:
                # 等待硬件 DAC 输出完成（Giảm xuống để phản hồi nhanh hơn）
    ...
    async def on_incoming_json(self, message: Any) -> None:
        ...
        try:
            ...
            if message.get("type") == "tts":
                ...
                if state == "start":
                    current_focus = (
                        self.codec.get_audio_focus() if self.codec else "idle"
                    )
                    clear_existing = current_focus in {"music", "system"}
                    if self.codec:
                        self.codec.set_audio_focus("tts")
                    await self._pause_music_for_tts(clear_existing=clear_existing)
                elif state == "stop":
                    if self.codec:
                        await self.codec.wait_output_drained(timeout=4.0)
                        await self.codec.release_audio_focus("tts")
    ...
    async def on_incoming_audio(self, data: bytes) -> None:
        ...
        if self.codec:
            try:
                if self.app and hasattr(self.app, "should_play_incoming_audio"):
                    if not self.app.should_play_incoming_audio():
                        return
                await self.codec.write_audio(data)
            except Exception as e:
                logger.debug(f"写入音频数据失败: {e}")

    async def _pause_music_for_tts(self, clear_existing: bool = True):
        """
        TTS 开始时：先清空音频队列，再暂停音乐.
        """
        try:
            if self.codec and clear_existing:
                await self.codec.clear_audio_queue()
                logger.debug("TTS 开始，已清空旧音频队列")
    ...
    async def _resume_music_after_tts(self):
        ...
        try:
            try:
                from src.mcp.tools.youtube.youtube_player import (
                    get_youtube_player_instance,
                )

                youtube_player = get_youtube_player_instance()
                if await youtube_player.play_deferred_if_ready():
                    logger.info("TTS finished, started deferred YouTube playback")
                    return
            except Exception as e:
                logger.warning(f"Starting deferred YouTube playback failed: {e}")

            try:
                from src.mcp.tools.radio.radio_player_v2 import (
                    get_radio_player_instance,
                )

                radio_player = get_radio_player_instance()
                if await radio_player.play_deferred_if_ready():
                    logger.info("TTS finished, started deferred radio playback")
                    return
            except Exception as e:
                logger.warning(f"Starting deferred radio playback failed: {e}")
    ...
    def mute_microphone(self, duration: float = 0.8):
        """Tạm thời tắt mic trong một khoảng thời gian (giây)."""
        import time
        self._mute_until = time.time() + duration
        logger.info(f"Mic đã được tạm tắt trong {duration}s để tránh lặp âm.")
