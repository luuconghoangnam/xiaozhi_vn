...
from .status import get_computer_status
...
class SystemToolsManager:
    ...
    def init_tools(self, add_tool, PropertyList, Property, PropertyType):
        ...
        try:
            ...
            # 注册计算机状态工具
            self._register_computer_status_tool(
                add_tool, PropertyList
            )

            # 注册 Shell 命令工具
            self._register_shell_command_tool(
                add_tool, PropertyList, Property, PropertyType
            )
    ...
    def _register_computer_status_tool(self, add_tool, PropertyList):
        """
        注册计算机状态工具.
        """
        add_tool(
            (
                "system.get_computer_status",
                "Lấy thông tin trạng thái máy tính (CPU, RAM, Pin, Ổ đĩa). "
                "Dùng khi người dùng hỏi về tình trạng máy, máy có lag không, pin còn bao nhiêu, v.v.",
                PropertyList(),
                get_computer_status,
            )
        )
        logger.debug("[SystemManager] 注册计算机状态工具成功")

    def _register_shell_command_tool(self, add_tool, PropertyList, Property, PropertyType):
        """
        注册 Shell 命令工具.
        """
        shell_props = PropertyList([Property("command", PropertyType.STRING)])
        add_tool(
            (
                "system.run_command",
                "Thực thi lệnh shell/terminal trên máy tính. Dùng để chạy các lệnh hệ thống, "
                "kiểm tra file, hoặc thực hiện các tác vụ kỹ thuật mà người dùng yêu cầu.",
                shell_props,
                run_shell_command,
            )
        )
        logger.debug("[SystemManager] 注册 Shell 命令工具成功")
