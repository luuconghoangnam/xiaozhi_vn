...
class MusicPlayer:
    ...
    def _get_library_dirs(self):
        dirs = []
        # default: cache_dir/music folder
        dirs.append(self.cache_dir)

        extra = os.getenv("XIAOZHI_MUSIC_DIRS", "").strip()
        if extra:
    ...
    async def get_local_playlist(self, force_refresh: bool = False) -> dict:
        ...
        try:
            ...
            for metadata in playlist:
                ...
                formatted_playlist.append(f"{title} - {artist}")
    ...
    async def _handle_playback_finished(self):
        ...
        if self.is_playing:
            ...
            if self.app and hasattr(self.app, "set_chat_message"):
                ...
                await self._safe_update_ui(f"Đã phát xong: {self.current_song} [{dur_str}]")
    ...
    async def search_and_play(self, song_name: str) -> dict:
        ...
        """
        try:
            # 🔴 PHIÊN BẢN VIỆT NAM: Ưu tiên dùng YouTube để có nhạc đầy đủ
            logger.info(f"Đang tìm bài hát '{song_name}' trên YouTube để đảm bảo chất lượng...")
            from src.mcp.tools.youtube.youtube_player import get_youtube_player_instance
            yt_player = get_youtube_player_instance()
            return await yt_player.search_and_play(song_name)
            ...
            if success:
                ...
                return {"status": "error", "message": "Phát nhạc thất bại"}
    ...
    async def _start_playback(
        ...
        try:
            ...
            if (
                self.app
                and (self.app.is_speaking() or getattr(self.audio_codec, "get_audio_focus", lambda: "idle")() == "tts")
            ):
    ...
    async def _write_to_audio_codec(self, pcm_data: np.ndarray):
        ...
        try:
            ...
            await self.audio_codec.write_pcm_direct(pcm_data, source="music")
