...
class MusicToolsManager:
    ...
    def _register_search_and_play_tool(
        ...
        add_tool(
            (
                ...
                "Tìm kiếm và phát bài hát. Tự động tìm kiếm trực tuyến theo tên bài hát và bắt đầu phát."
                "Nếu đang có nhạc phát, sẽ dừng nhạc cũ và phát nhạc mới."
                "Dùng khi người dùng yêu cầu bài hát cụ thể, ví dụ: 'phát bài Lạc Trôi', 'nghe nhạc Sơn Tùng'.",
                search_props,
    ...
    def _register_pause_tool(self, add_tool, PropertyList):
        ...
        add_tool(
            (
                "music_player.pause",
                "Tạm dừng nhạc đang phát. Giữ nguyên vị trí hiện tại."
                "Dùng khi người dùng nói 'tạm dừng', 'dừng nhạc một chút'.",
                PropertyList(),
                pause_wrapper,
            )
        )
    ...
    def _register_stop_tool(self, add_tool, PropertyList):
        ...
        async def stop_wrapper(args: Dict[str, Any]) -> str:
            ...
            return result.get("message", "Đã dừng nhạc")

        add_tool(
            (
                "music_player.stop",
                "Dừng hẳn việc phát nhạc. Dùng khi người dùng nói 'tắt nhạc', 'dừng phát'.",
                PropertyList(),
                stop_wrapper,
            )
        )
