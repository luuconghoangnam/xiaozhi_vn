...
class AudioCodec:
    ...
    def set_audio_focus(self, source: str) -> None:
        """Set the active output owner: idle, tts, music, or system."""
        if source not in {"idle", "tts", "music", "system"}:
            source = "idle"
        if self._audio_focus != source:
            logger.info(f"音频输出焦点切换: {self._audio_focus} -> {source}")
            self._audio_focus = source

    def get_audio_focus(self) -> str:
        return self._audio_focus

    def get_output_pending_frames(self) -> int:
        try:
            return self._output_buffer.qsize() + len(self._resample_output_buffer)
        except Exception:
            return 0

    async def wait_output_drained(self, timeout: float = 4.0) -> bool:
        deadline = asyncio.get_running_loop().time() + timeout
        while self.get_output_pending_frames() > 0:
            if asyncio.get_running_loop().time() >= deadline:
                logger.debug(
                    f"等待音频播放队列清空超时，剩余: {self.get_output_pending_frames()}"
                )
                return False
    ...
    async def write_pcm_direct(self, pcm_data: np.ndarray, source: str = "music"):
        ...
        """
        try:
            if self._audio_focus == "tts" and source != "tts":
                return
            if self._audio_focus == "idle":
                self._audio_focus = source if source in {"music", "system"} else "music"
            elif self._audio_focus != source:
                return
    ...
    async def release_audio_focus(self, source: str | None = None):
        if source is None or self._audio_focus == source:
            self._audio_focus = "idle"
