...
class MusicDecoder:
    ...
    async def start_decode(
        ...
        is_url = str(file_path).startswith(("http://", "https://", "rtmp://", "rtsp://"))
        
        if not is_url and not Path(file_path).exists():
            logger.error(f"Tập tin âm thanh không tồn tại: {file_path}")
            return False
        ...
        try:
            ...
            display_name = getattr(file_path, "name", str(file_path))
            if is_url:
                # Nếu là URL dài, chỉ hiện phần đầu cho gọn log
                display_name = display_name[:50] + "..." if len(display_name) > 50 else display_name

            logger.info(
                f"Bắt đầu giải mã âm thanh: {display_name}{position_info} "
