...
class CliDisplay(BaseDisplay):
    ...
    async def update_details(self, status: str, task: str, response: str, meta: str):
        try:
            lines = [x for x in [status, task, response, meta] if x]
            if lines:
                self._dash_text = " | ".join(lines[:3])
        except Exception:
            pass
            ...
            while self.running:
                ...
                if self._use_ansi:
                    ...
                    try:
                        ...
                        # Headless/background run: stdin may be closed -> avoid exiting
    ...
    def _print_help(self):
        """
        将Trợ giúp信息写入顶部内容显示区，而非直接打印。
        """
        help_text = "r: Bắt đầu/Dừng | x: Ngắt | q: Thoát | h: Trợ giúp | 其他: Gửi văn bản"
    ...
    def _redraw_input_line(self, content: str) -> None:
        ...
        prompt = "Nhập: " if not self._use_ansi else "\x1b[1m\x1b[36m输入:\x1b[0m "
    ...
    async def _render_dashboard(self, full: bool = False):
        ...
        lines = [
            ...
            f"Cảm xúc: {trunc(self._dash_emotion)}",
            f"Nội dung: {trunc(self._dash_text)}",
        ]
        ...
        title = style(" Xiaozhi AI (CLI) ", "bold", "cyan")
        # 头部框和底部框
