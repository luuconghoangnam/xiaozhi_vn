...
# --- OpenGL/QtQuick compatibility (Windows) ---
# Must be set BEFORE creating QApplication/QQuickWidget.
try:
...
class QmlLoggingHandler(logging.Handler):
    """
    将 Python 日志重定向到 QML 数据模型的处理器.
    """
    ...
    def emit(self, record):
        try:
            ...
            # 调度到主线程更新 UI（通过 signals）
            self.display_model.append_log(msg)
        except Exception:
...
class GuiDisplay(BaseDisplay, QObject, metaclass=CombinedMeta):
    ...
    async def update_emotion(self, emotion_name: str):
        ...
        self.display_model.update_emotion(url_or_text, emotion_name)

    async def update_details(self, status: str, task: str, response: str, meta: str):
        try:
            self.display_model.update_details(status, task, response, meta)
            if self.system_tray:
                tip_parts = []
                if status:
                    tip_parts.append(status)
                if task:
                    tip_parts.append(task)
                tooltip = " | ".join(tip_parts[:2]) if tip_parts else status
                if tooltip:
                    self.system_tray.update_status(tooltip, self.is_connected)
        except Exception as e:
            self.logger.debug(f"update_details failed: {e}")
    ...
    async def toggle_window_visibility(self):
        ...
        if self.root.isVisible():
            self.logger.debug("Ẩn cửa sổ (phím tắt)")
            self.root.hide()
        else:
            self.logger.debug("Hiện cửa sổ (phím tắt)")
    ...
    def _load_qml(self):
        ...
        加载 QML 界面.
        """
        # Force software rendering to avoid EGL/OpenGL context errors (Windows).
        try:
            QQuickWindow.setSceneGraphBackend(QQuickWindow.Software)
        except Exception:
            pass
    ...
    def _setup_logging(self):
        ...
        try:
            ...
            self.log_handler = QmlLoggingHandler(self.display_model)
            root_logger.addHandler(self.log_handler)
            self.logger.info("GUI 日志重定向已启动")
        except Exception as e:
            self.logger.error(f"设置 GUI 日志失败: {e}")
    ...
    async def _finalize_startup(self):
        ...
        # 根据配置决定显示模式
        start_minimized = os.getenv("XIAOZHI_START_MINIMIZED", "1") == "1"
        ...
        if start_minimized and self.system_tray:
            self.root.hide()
    ...
    def _on_mode_button_click(self):
        ...
        mode_text = "Tự động" if self.auto_mode else "Thủ công"
